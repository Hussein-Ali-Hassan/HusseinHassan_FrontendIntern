...
export default function SingleChoice({ sectionIndex, questionIndex }) {
  ...
  const handleOptionsChange = (event) => {
    ...
    if (event.target.name === "answer")
      ...
      // set all radio btns to false
      // since when checking a radio btn, the other btns don't change to false
      newOptions.forEach((option) => (option.isCorrect = false));

      newOptions[event.target.dataset.id].isCorrect = event.target.checked;
    }
